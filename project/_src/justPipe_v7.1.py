...
class MainWindow(QtWidgets.QMainWindow, _UI_Control.Ui_CONTROL):
    ...
    def loadprof(self, fName):
        if fName:
            ...
            self.setWindowTitle(f'v.7.1 Control - {os.path.basename(self.profName)}')
            AutoPipe()
    ...
    def loadwork(self, fName):
        ...
        self.setWindowTitle(f'v.7.1 Control - {os.path.basename(self.profName)[:-4]}')
...
class LV(QtWidgets.QMainWindow, _UI_Lview.Ui_LVIEW):
    def __init__(self):
        ...
        self.t_aspect.setText(f'{1 / self.aspect:.2f}')
        ...
        # connect
        self.t_aspect.textEdited.connect(self.set_vscale)
    ...
    def set_vscale(self):
        self.aspect = 1 / float(self.t_aspect.text())
        self.l_scale.setText(f'SCALE 1:{1 / self.aspect:.2f}')
        self.lview.setAspectLocked(True, self.aspect)

        if float(self.t_aspect.text()) != 1:
...
def key_pressed(e):
    ...
    # LView aspect
    if e.key() in [Qt.Key_Plus, Qt.Key_Equal, Qt.Key_Minus, Qt.Key_Underscore]:
        if e.key() in [Qt.Key_Plus, Qt.Key_Equal]:
            lv.ch_Aspect.setChecked(False)
            lv.aspect /= 1.5
        if e.key() in [Qt.Key_Minus, Qt.Key_Underscore]:
            lv.ch_Aspect.setChecked(False)
            lv.aspect *= 1.5
        lv.l_scale.setText(f'SCALE 1:{1 / lv.aspect:.2f}')
        lv.t_aspect.setText(f'{1 / lv.aspect:.2f}')
        lv.lview.setAspectLocked(True, lv.aspect)
